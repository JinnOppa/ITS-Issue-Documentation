...
def normalize_content_with_seq(content):
    """
    Ensure every content block has a stable seq.
    Backward compatible: old versions without seq will be auto-assigned.
    """
    normalized = []
    seq = 1
    for blk in content:
...
#     # If after attempting to copy referenced/staged images the new images folder
#     # is still empty, preserve assets by copying all images from the most
#     # recent previous version that has an images folder. This makes sure that
#     # older images remain available even when the editor content doesn't
#     # explicitly reference them.
#     try:
#         if not os.listdir(imgs_folder):
#             for prev_v in range(new_version - 1, 0, -1):
#                 prev_imgs = os.path.join(VERSIONS_DIR, error_code, f"v{prev_v}", "images")
#                 if os.path.exists(prev_imgs) and os.listdir(prev_imgs):
#                     for fname in os.listdir(prev_imgs):
#                         src = os.path.join(prev_imgs, fname)
#                         dst = os.path.join(imgs_folder, fname)
#                         try:
...
def save_new_version(error_code, content_blocks, updated_by):
    ...
    content_blocks:
    [
        {"seq":1,"type":"text","value":"..."},
        {"seq":2,"type":"image","value":"img.png"}
    ]
    """
    ...
    if os.path.exists(staging):
        for fname in os.listdir(staging):
            shutil.move(
                ...
                os.path.join(imgs_folder, fname)
            )
        try:
            os.rmdir(staging)
        except:
    ...
    # --- ensure referenced images exist ---
    for blk in content_blocks:
        if blk["type"] != "image":
            continue
        ...
        for pv in range(new_version - 1, 0, -1):
            cand = os.path.join(
                VERSIONS_DIR, error_code, f"v{pv}", "images", fname
            )
            if os.path.exists(cand):
                try:
                    shutil.copy2(cand, dst)
    ...
    # --- fallback: copy all images if folder empty ---
    try:
        if not os.listdir(imgs_folder):
            for pv in range(new_version - 1, 0, -1):
                prev_imgs = os.path.join(
                    VERSIONS_DIR, error_code, f"v{pv}", "images"
                )
                if os.path.exists(prev_imgs) and os.listdir(prev_imgs):
                    for f in os.listdir(prev_imgs):
                        try:
                            shutil.copy2(
                                os.path.join(prev_imgs, f),
                                os.path.join(imgs_folder, f)
                            )
                        except:
                            pass
                    break
...
class AppMain:
    ...
    def load_version(self, version_num):
        ...
        content = normalize_content_with_seq(data.get("content", []))
        content = sorted(content, key=lambda x: x["seq"])
        ...
        # If no image blocks were present in data.json but the version's
        # images/ folder contains files, render those images (helps v5 case
        # where images exist on disk but were not saved as blocks).
        try:
            imgs_folder = os.path.join(VERSIONS_DIR, self.current_error, f"v{version_num}", "images")
            if not image_rendered and os.path.exists(imgs_folder):
                files = sorted([f for f in os.listdir(imgs_folder) if not f.startswith('.')])
                for fname in files:
                    ipath = os.path.join(imgs_folder, fname)
                    if os.path.exists(ipath):
                        try:
                            img = Image.open(ipath)
                            max_w, max_h = 700, 450
    ...
    def handle_save_version(self):
        ...
        # -------------------------------------------------
        # Load previous version (seq reference source)
        # -------------------------------------------------
        prev_content = []
        prev_image_seq_map = {}   # image matched by filename
        prev_text_seqs = []       # text matched by POSITION

        prev_version = get_latest_version_num(ecode)
        if prev_version > 0:
            prev_data = load_version_data(ecode, prev_version)
            if prev_data:
                prev_content = normalize_content_with_seq(
                    prev_data.get("content", [])
                )

                for blk in prev_content:
                    if blk["type"] == "image":
                        prev_image_seq_map[blk["value"]] = blk["seq"]
                    elif blk["type"] == "text":
                        prev_text_seqs.append(blk["seq"])

        # -------------------------------------------------
        # Parse editor content
        # TEXT → positional seq
        # IMAGE → filename-based seq
        # -------------------------------------------------
        ...
        blocks = []
        text_idx = 0
        next_seq = max([b["seq"] for b in prev_content], default=0) + 1
        ...
        for ln in lines:
            ...
            if ln.startswith("[IMAGE:") and ln.endswith("]"):
                ...
                if fname in prev_image_seq_map:
                    seq = prev_image_seq_map[fname]
                else:
                    seq = next_seq
                    next_seq += 1
            ...
            # ---------- TEXT ----------
                ...
                if text_idx < len(prev_text_seqs):
                    # positional match → KEEP SEQ
                    seq = prev_text_seqs[text_idx]
                else:
                    # new text block
                    seq = next_seq
                    next_seq += 1
        ...
        # -------------------------------------------------
        # Preserve previous images not referenced in editor
        # (editor tidak serialize image)
        # -------------------------------------------------
        current_image_seqs = {
            blk["seq"] for blk in blocks
            if blk["type"] == "image"
        }

        for blk in prev_content:
            if blk["type"] == "image" and blk["seq"] not in current_image_seqs:
                blocks.append(blk)

        # -------------------------------------------------
        # Final ordering (CRITICAL)
        # -------------------------------------------------
        blocks = sorted(blocks, key=lambda x: x["seq"])

        # -------------------------------------------------
        # Save new version
        # -------------------------------------------------
        new_vnum = save_new_version(
            error_code=ecode,
        ...
        messagebox.showinfo(
            "Saved",
            f"Saved new version v{new_vnum} for {ecode}"
        )
    ...
    #     # -------------------------------------------------
    #     # Load previous version (for seq + image preservation)
    #     # -------------------------------------------------
    #     prev_content = []
    #     prev_seq_map = {}
    ...
    #         prev_data = load_version_data(ecode, prev_version)
    #         if prev_data:
    #             prev_content = normalize_content_with_seq(prev_data.get("content", []))
    #             for blk in prev_content:
    #                 key = (blk["type"], blk["value"])
    #                 prev_seq_map[key] = blk["seq"]

    #     # -------------------------------------------------
    #     # Parse editor text (TEXT is source of truth)
    #     # -------------------------------------------------
    #     raw = self.txt.get("1.0", "end").strip()
    #     lines = raw.splitlines()
    ...
    #             key = ("image", fname)
    #             if key in prev_seq_map:
    #                 seq = prev_seq_map[key]
    #             else:
    #                 seq = next_seq
    #                 next_seq += 1
    ...
    #     # (image tidak ikut di editor text → jangan hilang)
    #     # -------------------------------------------------
    #     prev_images = [
    #         blk for blk in prev_content
    #         if blk["type"] == "image"
    #     ]
    ...
    #     for img_blk in prev_images:
    #         if img_blk["seq"] not in current_image_seqs:
    #             blocks.append(img_blk)
        
    #     # -------------------------------------------------
    #     # Final sort by seq (CRITICAL)
    #     # -------------------------------------------------
    #     blocks = sorted(blocks, key=lambda x: x["seq"])
