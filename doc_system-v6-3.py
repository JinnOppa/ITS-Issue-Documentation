...
# Tambahkan di bagian atas bersama CONFIG lainnya
PDF_DIR = os.path.join(DATA_DIR, "user_guide_pdf")
...
def ensure_files():
    ...
    os.makedirs(PDF_DIR, exist_ok=True) # Tambahkan ini
...
class LoginWindow:
    def __init__(self):
        ...
        # Ambil dimensi layar (lebar & tinggi)
        screen_width = self.root.winfo_screenwidth()
        screen_height = self.root.winfo_screenheight()

        # Hitung titik koordinat X dan Y untuk posisi tengah
        center_x = int(screen_width/2 - window_width / 2)
        center_y = int(screen_height/2 - window_height / 2)

        # Set geometry dengan format: "lebar x tinggi + koordinat_x + koordinat_y"
        ...
        # Fitur Login dengan Enter
        self.root.bind('<Return>', lambda e: self.login())

        ttk.Button(self.root, text="Login", command=self.login).pack(pady=20)
        
        # Tambahkan ini agar jendela muncul paling depan saat di-run
...
class AppMain:
    def __init__(self, role, username):
        ...
        # --- MEMBUAT WINDOW LANGSUNG MAXIMIZED ---
        # Untuk Windows
        try:
            self.root.state('zoomed')
        except:
            # Untuk Linux/Mac jika 'zoomed' tidak tersedia
        ...
        # 1. Home Tab (Semua bisa lihat)
        self.build_home_tab()

        # 2. Documentation Tab (Semua bisa lihat - akses isi dibedakan di dalam fungsinya)
        self.build_doc_tab()
        
        # 3. Knowledge Base Tab (Semua bisa lihat)
        self.build_kb_tab()

        # 4. IT User Guide Tab (Hanya IT Admin & IT User)
        if self.role in ("it_admin", "it_user"):
            self.build_pdf_guide_tab()
            
        # 5. User Management (Hanya IT Admin & IT User)
        if self.role in ("it_admin", "it_user"):
            self.build_user_tab()
    ...
    def build_home_tab(self):
        ...
        lbl = ttk.Label(tab, text=f"Welcome to DocSystem, {self.username}!", font=("Arial", 16, "bold"))
        lbl.pack(pady=50)
        
        info = ttk.Label(tab, text=f"Role Anda: {self.role}\nSilakan pilih menu di atas untuk memulai.")
        info.pack()

    def build_kb_tab(self):
        tab = ttk.Frame(self.tabs)
        self.tabs.add(tab, text="📖 Knowledge Base")
        
        # Bisa menggunakan ScrolledText yang di-set state='disabled' (Read Only)
        kb_text = ScrolledText(tab, wrap="word")
        kb_text.pack(fill="both", expand=True, padx=10, pady=10)
        
        content = """
        === BASIC ACCOUNTING TERMS ===
        
        1. AP (Account Payable): Hutang perusahaan kepada supplier.
        2. AR (Account Receivable): Piutang/tagihan perusahaan kepada customer.
        3. GL (General Ledger): Buku besar yang mencatat semua transaksi.
        ...
        """
        kb_text.insert("1.0", content)
        kb_text.config(state="disabled") # Kunci agar tidak bisa diedit

    def build_pdf_guide_tab(self):
        tab = ttk.Frame(self.tabs)
        self.tabs.add(tab, text="📂 IT User Guide")
        
        # Layout: Treeview untuk list PDF
        cols = ("filename", "size")
        self.pdf_tree = ttk.Treeview(tab, columns=cols, show="headings")
        self.pdf_tree.heading("filename", text="File Name")
        self.pdf_tree.heading("size", text="Size")
        self.pdf_tree.pack(fill="both", expand=True, padx=10, pady=10)

        btn_frame = ttk.Frame(tab)
        btn_frame.pack(fill="x", padx=10, pady=5)

        # Logika Akses
        ttk.Button(btn_frame, text="📥 Download PDF", command=self.download_pdf).pack(side="left", padx=5)
        
        if self.role == "it_admin":
            ttk.Button(btn_frame, text="📤 Upload Guide", command=self.upload_pdf).pack(side="left", padx=5)
            ttk.Button(btn_frame, text="✏️ Rename", command=self.rename_pdf).pack(side="left", padx=5)
            ttk.Button(btn_frame, text="🗑 Delete", command=self.delete_pdf).pack(side="right", padx=5)

        # Tambahkan pemanggilan refresh di baris terakhir fungsi ini
    ...
    def build_doc_tab(self):
        ...
        if self.role == "it_admin":
            self.btn_edit_meta = ttk.Button(
                meta, 
                text="✏️ Edit Meta", 
                width=10,
                command=self.edit_metadata
            )
            self.btn_edit_meta.grid(row=0, column=2, padx=5, sticky="e")
    ...
    def refresh_pdf_list(self):
        ...
        # List file di folder PDF_DIR
        if os.path.exists(PDF_DIR):
            for filename in os.listdir(PDF_DIR):
                if filename.lower().endswith('.pdf'):
                    path = os.path.join(PDF_DIR, filename)
                    size = os.path.getsize(path) / 1024 # KB
                    self.pdf_tree.insert("", "end", iid=filename, values=(filename, f"{size:.2f} KB"))
    ...
    def upload_pdf(self):
        ...
        if os.path.exists(dest_path):
            if not messagebox.askyesno("Overwrite", "File sudah ada. Ganti file lama?"):
                return
        
        shutil.copy2(file_path, dest_path)
        messagebox.showinfo("Success", f"File {filename} berhasil diunggah.")
        self.refresh_pdf_list()

    def download_pdf(self):
        sel = self.pdf_tree.selection()
        if not sel:
            messagebox.showwarning("Warning", "Pilih file yang ingin di-download.")
            return
        
        filename = sel[0]
        src_path = os.path.join(PDF_DIR, filename)
        
        # Tanya user ingin simpan di mana
        ...
        if save_path:
            ...
            messagebox.showinfo("Success", "File berhasil disimpan.")
    ...
    def rename_pdf(self):
        ...
        old_name = sel[0]
        # Dialog sederhana untuk input nama baru
        new_name = tk.simpledialog.askstring("Rename", "Masukkan nama baru:", initialvalue=old_name)
        ...
        if new_name:
            ...
            try:
                os.rename(os.path.join(PDF_DIR, old_name), os.path.join(PDF_DIR, new_name))
                self.refresh_pdf_list()
    ...
    def delete_pdf(self):
        ...
        filename = sel[0]
        if messagebox.askyesno("Confirm", f"Hapus file {filename}?"):
            os.remove(os.path.join(PDF_DIR, filename))
            self.refresh_pdf_list()


    # --------------------------------------------------
    # DOC ACTIONS + IMAGE + RENDER + PREVIEW + VERSION CONTROL + EDIT METADATA
    # --------------------------------------------------
    ...
    def load_version(self, e=None):
        ...
        # IMAGE STATE
        raw_images = data.get("image_path", {})
        self.images = {}

        for key, path in raw_images.items():
            if os.path.exists(path):
                # 🔥 pakai filename asli dari file
                filename = os.path.basename(path)

                # rebuild path (optional tapi lebih clean)
                new_path = path  # atau normalize kalau mau

                self.images[key] = new_path
            else:
    ...
    def toggle_preview(self):
        ...
        if not self.preview_mode:
            # 1. Simpan apa yang sedang diketik ke buffer (termasuk referensi gambar baru)
            self._edit_buffer = self.txt.get("1.0", "end-1c")
            
            # 2. Aktifkan mode preview
            self.preview_mode = True
            self.btn_preview.config(text="✍️ Edit Mode") # Opsional: ubah teks tombol
            self.render_preview()
        else:
    ...
    def render_preview(self):
        # Bersihkan layar
        self.txt.delete("1.0", "end")
        self._img_refs = [] # Reset referensi agar tidak dimakan garbage collector

        # Gunakan teks dari buffer (yang baru diketik)
        content = self._edit_buffer 

        # Split konten berdasarkan pola {img_N}
            ...
            if re.match(r'\{img_\d+\}', part):
                ...
                if img_path and os.path.exists(img_path):
                    try:
                        ...
                        # Ukuran thumbnail bisa disesuaikan
                        ...
                        # PENTING: Simpan referensi objek image
                        self._img_refs.append(img)
                    except Exception as e:
                        self.txt.insert("end", f"[Error Memuat Gambar: {key}]")
                else:
                    self.txt.insert("end", f"[Gambar belum di-upload/tidak ditemukan: {key}]")
            else:
                # Masukkan teks biasa
                self.txt.insert("end", part)
        
        # Kunci teks agar tidak bisa diedit saat preview (opsional)
        self.txt.configure(state="normal") # Tetap normal agar image bisa muncul, tapi cegah input manual jika perlu
    ...
    def refresh_users(self):
        ...
        for u in self.users.values():
            # LOGIKA FILTER:
            # Jika saya adalah it_user, jangan tampilkan siapa pun yang rolenya it_admin
            if self.role == "it_user" and u["role"] == "it_admin":
                continue
